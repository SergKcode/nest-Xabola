...
import { Observable, catchError, forkJoin, from, of, switchMap, tap } from 'rxjs';
...
export class ProductsService {
	...
	constructor(
		...
		private _uploadImagesService: FirebaseImageService
	) {}
	 ...
	 * @param image
	 */
	createNewProduct(data: CreateProductDto, image: Express.Multer.File): Observable<Products> {
		const { typeId } = data;
		...
		return this._uploadImagesService.saveImage(image).pipe(
			switchMap((image) => {
				...
				if (!image) {
					throw new HttpException({ message: `No se añadió ninguna imagen` }, HttpStatus.BAD_REQUEST);
				}
				return this._getOneProductType(typeId).pipe(
					switchMap((typeId) => {
						const product = Object.assign({}, data, { image: url }, { typeId }, { fileName });
						console.log('product', product);
						return from(this._productsRepository.save(product));
					}),
					catchError((error) => {
						this._logger.error(`Error creando un nuevo producto`, error);
	...
	getAllProducts(): Observable<Products[]> {
		...
		return from(this._productsRepository.find()).pipe(
			catchError((error) => {
				this._logger.error(`Error obteniendo productos`, error);
	...
	private _getOneProductType(id: string): Observable<ProductTypes> {
		...
		return from(this._productsTypeRepository.findOne({ where: { id } })).pipe(
			catchError((error) => {
				...
				throw new HttpException({ message: `Typo de producto no encontrado: ${error}` }, HttpStatus.NOT_FOUND);
			})
		);
	}
	...
	updateProduct(id: string, product: UpdateProductDto): Observable<any> {
		...
		return forkJoin([from(this.getOneProduct(id)), typeId ? from(this._getOneProductType(typeId)) : of({})]).pipe(
			switchMap(([product, type]) => {
				if (!product) {
					throw new HttpException({ message: `Producto con id: ${id}, no encontrado` }, HttpStatus.NOT_FOUND);
				}
	...
	getProductsByType(id: string): Observable<Products[]> {
		...
		return from(this._productsTypeRepository.findOne({ where: { id } })).pipe(
			switchMap((typeId) => from(this._productsRepository.find({ where: { typeId } })))
		);
	}
	 ...
	 */
	createTypeProduct(newProductType: CreateProductTypesDto): Observable<ProductTypes> {
