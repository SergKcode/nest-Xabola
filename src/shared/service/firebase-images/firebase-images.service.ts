...
export class FirebaseImageService {
	private readonly _logger = new Logger(FirebaseImageService.name, {
		timestamp: true
	});
	...
	saveImage(image: Express.Multer.File) {
		...
		return from(
			...
			catchError((e) => {
				console.error(e);
				throw new HttpException({ message: `Error subiendo imagen` }, HttpStatus.BAD_REQUEST);
			})
		);
	}

	/**
	 * Metodo para eliminar imagenes de firebase
	 * @param image 
	 * @returns 
	 */
	deleteImage(fileName: string): Observable<any> {
		...
		return from(file.delete()).pipe(
			catchError((e) => {
				console.error(e);
				throw new HttpException({ message: `Error eliminando la imagen` }, HttpStatus.BAD_REQUEST);
			})
		);
	}
}
